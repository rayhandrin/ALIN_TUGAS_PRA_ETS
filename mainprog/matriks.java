...
public class matriks {
    ...
    public void TulisFile(int type, double hasil, String solusi) {
        ...
        // type = 2 untuk menulis hasil (bisa berupa determinan ataupun hasil
        // interpolasi)
        Scanner in = new Scanner(System.in);
    ...
    public int LeftestOne(int a) {
        for (int i = 1; i < this.kolom; i++) {
            if (Math.abs(this.Mat[a][i] - 1) <= 0.0001)
                return i;
        }
        return -1;
    }
    ...
    // Fungsi untuk mengembalikan determinan matriks persegi yang akan digunakan
    // untuk kofaktor
